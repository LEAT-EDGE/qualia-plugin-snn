...
class QuantizedBasicBlock(nn.Module):
    ...
    QuantizedMaxPool layer will be absent if ``pool_size == 1``.

    Residual (right) branch QuantizedConv layer will be asbent if ``in_planes==planes``, except if
    ``force_projection_with_pooling==True`` and ``pool_size != 1``.
    """
    ...
    def __init__(self,  # noqa: PLR0913
        ...
        :param force_projection_with_pooling: If ``True``, residual QuantizedConv layer is kept when ``stride != 1`` even if
                                             ``in_planes == planes``
        ...
        if self.pool_size != 1:
            self.pool1 = sjlayers_t.QuantizedMaxPool(pool_size, step_mode=step_mode, quant_params=quant_params, activation=None)
        ...
        if self.pool_size != 1:
            self.spool = sjlayers_t.QuantizedMaxPool(pool_size, step_mode=step_mode, quant_params=quant_params)
...
class QuantizedSResNetStride(SNN):
    """Quantized residual spiking neural network template.

    Should have topology identical to :class:`qualia_plugin_snn.learningmodel.pytorch.SResNetStride.SResNetStride` but with layers
    ...
    def __init__(self,  # noqa: C901, PLR0912, PLR0913
        ...
        :param filters: List of ``out_channels`` for QuantizedConv layers inside each :class:`QuantizedBasicBlock` group, must be
                        be of the same size as ``num_blocks``, first element is for the first QuantizedConv layer at the beginning
                        of the network
        :param kernel_sizes: List of ``kernel_size`` for QuantizedConv layers inside each :class:`QuantizedBasicBlock` group, must
                             be of the same size as ``num_blocks``, first element is for the first QuantizedConv layer at the
                             beginning of the network
        :param pool_size: List of ``kernel_size`` for QuantizedMaxPool layers inside each :class:`QuantizedBasicBlock` group, must
                        be of the same size as ``num_blocks``, ``stride`` is applied only to the first :class:`QuantizedBasicBlock`
        ...
        :param strides: List of ``stride`` for first QuantizedConv layers inside each :class:`QuantizedBasicBlock` group, must
                        be of the same size as ``num_blocks``, first element is for the first Conv layer at the beginning of
                        the network
        :param paddings: List of ``padding`` for QuantizedConv layer inside each :class:`QuantizedBasicBlock` group, must be of the
        ...
        if pool_sizes[0] != 1:
            self.pool1 = sjlayers_t.QuantizedMaxPool(pool_sizes[0], step_mode=self.step_mode, quant_params=quant_params)
        ...
        if postpool == 'sum':
            self.conv2 = sjlayers_t.QuantizedConv(in_channels=filters[-1],
            ...
            self.gsp = sjlayers_t.QuantizedGlobalSumPool(quant_params=quant_params, step_mode=self.step_mode)
        else:
            ...
            if postpool == 'avg':
                self.postpool = sjlayers_t.QuantizedAvgPool(tuple(self._fm_dims),
                                                            step_mode=self.step_mode,
                                                            quant_params=quant_params)
            elif postpool == 'max':
                self.postpool = sjlayers_t.QuantizedMaxPool(tuple(self._fm_dims),
                                                            step_mode=self.step_mode,
                                                            quant_params=quant_params)
