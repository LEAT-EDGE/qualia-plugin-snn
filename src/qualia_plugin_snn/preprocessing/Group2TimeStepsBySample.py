...
from qualia_core.preprocessing.Preprocessing import Preprocessing, iterate_generator
...
class Group2TimeStepsBySample(Preprocessing[RawDataModel, RawDataModel]):
    ...
    @iterate_generator
    def __process_set(self, s: RawData, sname: str) -> RawData:
        if s.info is None:
    ...
    def __call__(self, datamodel: RawDataModel) -> RawDataModel:
        ...
        for name, s in datamodel:
            ...
            if isinstance(s, RawDataChunks) and s.shapes.data is not None:
                # Add the timesteps dimension
                # Total number of samples after transformation cannot be known at this time so leave undefined
                s.shapes.data = (None, self.__timesteps, *s.shapes.data[1:])

            setattr(datamodel.sets, name, self.__process_set(s, name))
